...
void ImGuiAl::Crt::setForegroundColor(ImU32 const color) { _foregroundColor = color; }

void ImGuiAl::Crt::setMetaData(unsigned const meta_data) { _metaData = meta_data; }
...
void ImGuiAl::Crt::scrollToBottom() { _scrollToBottom = true; }

void ImGuiAl::Crt::clear() { _fifo.reset(); }

void ImGuiAl::Crt::iterate(
    const std::function<bool(Info const& header, char const* const line)>& iterator) const
{
    ...
    while (pos < occupied)
    {
        ...
        if (header.length >= max_length)
        {
            ...
            char* new_line =
                static_cast<char*>(line == temp ? malloc(max_length) : realloc(line, max_length));
...
{
    draw(size,
         [](Info const& header, char const* const line) -> bool
         {
             (void)header;
             (void)line;
             return true;
                        ...
                        const std::function<bool(Info const& header, char const* const line)>& filter)
{
    ...
    iterate(
        [&filter](Info const& header, char const* const line) -> bool
        {
...
{
    constexpr float hBlue = 130.f / 255.f, hGreen = 94.f / 255.f, hRed = 0.f, hYellow = 42.f / 255.f;
    constexpr float hBlueHovered = 117.f / 255.f, hGreenHovered = 56.f / 255.f, hRedHovered = 228.f / 255.f,
                    hYellowHovered = 36.f / 255.f;
...
ImU32 makeColor(LogColorClass logColorClass, LogColorState logColorState, float windowBgHsv_Value)
{
    ...
    ImGui::ColorConvertHSVtoRGB(
        resultHsv[0], resultHsv[1], resultHsv[2], resultRgb[0], resultRgb[1], resultRgb[2]);
...
void ImGuiAl::Log::setColorsAutoFromWindowBg()
{
    ...
    ImGui::ColorConvertRGBtoHSV(
        windowBgColor.x, windowBgColor.y, windowBgColor.z, windowBgHsv[0], windowBgHsv[1], windowBgHsv[2]);
    ...
    _debugButtonHoveredColor =
        makeColor(LogColorClass::Debug_Blue, LogColorState::ButtonHovered, windowBgHsv_Value);
    ...
    _infoButtonHoveredColor =
        makeColor(LogColorClass::Info_Green, LogColorState::ButtonHovered, windowBgHsv_Value);
    ...
    _warningButtonHoveredColor =
        makeColor(LogColorClass::Warning_Yellow, LogColorState::ButtonHovered, windowBgHsv_Value);
    ...
    _errorButtonHoveredColor =
        makeColor(LogColorClass::Error_Red, LogColorState::ButtonHovered, windowBgHsv_Value);
    _errorTextColor = makeColor(LogColorClass::Error_Red, LogColorState::Text, windowBgHsv_Value);
}

int ImGuiAl::Log::draw(ImVec2 const& size, bool minimal)
{
    ...
    if (minimal)
    {
        Crt::draw(size, [this](Info const& header, char const* const line) -> bool { return true; });
        return action;
    }
    ...
    Crt::draw(size,
              [this](Info const& header, char const* const line) -> bool
              {
...
void ImGuiAl::Log::setCumulativeLabel(char const* const label) { _cumulativeLabel = label; }

void ImGuiAl::Log::setFilterLabel(char const* const label) { _filterLabel = label; }

void ImGuiAl::Log::setFilterHeaderLabel(char const* const label) { _filterHeaderLabel = label; }

void ImGuiAl::Log::setActions(char const* actions[]) { _actions = actions; }
