...
namespace HelloImGuiIniSettings
{
    ...
    {
        std::string r =
            ";;; !!! This configuration is handled by HelloImGui and stores several Ini Files, separated by "
            "markers like this:\n           ;;;<<<INI_NAME>>>;;;\n\n";
    ...
    std::optional<float> LoadLastRunDpiWindowSizeFactor(const std::string& iniPartsFilename)
    {
        ...
        if (appWindowSection.find("DpiWindowSizeFactor") != appWindowSection.end())
        {
            ...
            bool isDpiSane =
                (dpiWindowSizeFactor_WhenSaved >= 0.1f) && (dpiWindowSizeFactor_WhenSaved <= 10.f);
            if (isDpiSane)
    ...
    void SaveImGuiSettings(const std::string& iniPartsFilename, const std::string& layoutName)
    {
        ...
        IniParts iniParts = IniParts::LoadFromFile(iniPartsFilename);
        if (!imguiSettingsContent.empty())
            iniParts.SetIniPart(iniPartName, imguiSettingsContent);

        iniParts.WriteToFile(iniPartsFilename);
    }

    bool HasUserDockingSettingsInImguiSettings(const std::string& iniPartsFilename,
                                               const DockingParams& dockingParams)
    {
        ...
        {
            if (std::find(windowsWithSettings.begin(), windowsWithSettings.end(), dockableWindow->label) ==
    ...
    static void SaveDockableWindowsVisibilityRec(
        ini::IniFile& iniFile, const std::vector<std::shared_ptr<DockableWindow>>& dockableWindows)
    {
    ...
    void SaveHelloImGuiMiscSettings(const std::string& iniPartsFilename, const RunnerParams& runnerParams)
    {
        ...
        if (runnerParams.imGuiWindowParams.rememberTheme)
            iniFile["Theme"]["Name"] =
                ImGuiTheme::ImGuiTheme_Name(runnerParams.imGuiWindowParams.tweakedTheme.Theme);
    ...
    void SaveUserPref(const std::string& iniPartsFilename,
                      const std::string& userPrefName,
                      const std::string& userPrefContent)
    {
        return;
        IniParts iniParts = IniParts::LoadFromFile(iniPartsFilename);
