...
HelloImGuiContext* CreateContext();
void DestroyContext(HelloImGuiContext* ctx = nullptr);  // NULL = destroy current context
HelloImGuiContext* GetCurrentContext();
void SetCurrentContext(HelloImGuiContext* ctx);
...
* `HelloImGui::Run(guiFunction, windowTitle, windowSize, windowSizeAuto=false,
restoreLastWindowGeometry=false, fpsIdle=10)` Runs an application, by providing the Gui function, the window
title, etc.
...
{
    // HelloImGui::ManualRender is a namespace that groups functions, allowing fine-grained control over the
    // rendering process:
    ...
    //        // runnerParams.fpsIdling.fpsIdlingMode = HelloImGui::FpsIdlingMode::Sleep; // or EarlyReturn,
    //        Auto HelloImGui::ManualRender::SetupFromRunnerParams(runnerParams); while
    ...
    //     By default,
    //       - On Emscripten, `ManualRender::Render()` will return immediately to avoid blocking the main
    //       thread.
    ...
    // This will initialize the platform backend (SDL, Glfw, etc.) and the rendering backend (OpenGL, Vulkan,
    // etc.). A distinct copy of `RunnerParams` is stored internally.
    ...
    // This will release the platform backend (SDL, Glfw, etc.) and the rendering backend (OpenGL, Vulkan,
    // etc.). After calling `TearDown()`, the InitFromXXX can be called with new parameters.
    void TearDown();
}  // namespace ManualRender
...
void AddDockableWindow(std::shared_ptr<DockableWindow> dockableWindow, bool forceDockspace = false);
