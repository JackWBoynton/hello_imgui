...
function(him_add_hello_imgui)
      ...
      target_include_directories(${HELLOIMGUI_TARGET} PUBLIC $<BUILD_INTERFACE:${CMAKE_CURRENT_SOURCE_DIR}/../src>)
...
function(him_install_imgui)
    if(HELLOIMGUI_INSTALL)
        ...
        install(TARGETS imgui DESTINATION ./lib/ COMPONENT Development)
        file(GLOB imgui_headers
            ${HELLOIMGUI_IMGUI_SOURCE_DIR}/*.h
            ${HELLOIMGUI_IMGUI_SOURCE_DIR}/misc/cpp/*.h
        )
        install(FILES ${imgui_headers} DESTINATION include COMPONENT Development)

        if(HELLOIMGUI_HAS_OPENGL3)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/backends/imgui_impl_opengl3.h DESTINATION include COMPONENT Development)
        endif()
        if(HELLOIMGUI_HAS_METAL)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/backends/imgui_impl_metal.h DESTINATION include COMPONENT Development)
        endif()
        if(HELLOIMGUI_HAS_VULKAN)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/backends/imgui_impl_vulkan.h DESTINATION include COMPONENT Development)
        endif()
        if(HELLOIMGUI_HAS_DIRECTX11)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/backends/imgui_impl_dx11.h DESTINATION include COMPONENT Development)
        endif()

        if(HELLOIMGUI_USE_SDL2)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/backends/imgui_impl_sdl2.h DESTINATION include COMPONENT Development)
        endif()
        if(HELLOIMGUI_USE_GLFW3)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/backends/imgui_impl_glfw.h DESTINATION include COMPONENT Development)
        endif()

        if(HELLOIMGUI_USE_FREETYPE)
            install(FILES ${HELLOIMGUI_IMGUI_SOURCE_DIR}/misc/freetype/imgui_freetype.h DESTINATION include COMPONENT Development)
        endif()
    endif()
endfunction()
...
function(him_install_implot)
    if(HELLOIMGUI_INSTALL)
        ...
        install(TARGETS implot DESTINATION ./lib/ COMPONENT Development)
...
function(him_add_stb_image)
    ...
    if(HELLOIMGUI_INSTALL AND NOT Stb_FOUND)
        ...
        install(FILES ${stb_headers} DESTINATION include COMPONENT Development)
    endif()
...
function(_him_add_glad)
    ...
    if(HELLOIMGUI_INSTALL)
        install(TARGETS glad DESTINATION ./lib/ COMPONENT Development)
        install(FILES ${glad_dir}/include/glad/glad.h DESTINATION include/glad COMPONENT Development)
        install(FILES ${glad_dir}/include/KHR/khrplatform.h DESTINATION include/KHR COMPONENT Development)
    endif()
endfunction()
...
function(him_install)
    if (HELLOIMGUI_INSTALL AND NOT IOS AND NOT ANDROID)
        install(TARGETS ${HELLOIMGUI_TARGET} DESTINATION lib/ COMPONENT Development)
        file(GLOB headers *.h)
        install(FILES ${headers} DESTINATION include/hello_imgui/ COMPONENT Development)
        file(GLOB internal_headers internal/*.h)
        install(FILES ${internal_headers} DESTINATION include/hello_imgui/internal COMPONENT Development)

        if(CMAKE_BUILD_TYPE STREQUAL "Release")
            install(DIRECTORY ${HELLOIMGUI_BASEPATH}/hello_imgui_cmake DESTINATION share/${PROJECT_NAME} COMPONENT Development)
            install(DIRECTORY ${HELLOIMGUI_BASEPATH}/hello_imgui_assets DESTINATION share/${PROJECT_NAME} COMPONENT Development)
            if (NOT IOS AND NOT ANDROID)
                install(FILES ${HELLOIMGUI_BASEPATH}/README.md DESTINATION share/${PROJECT_NAME} COMPONENT Development)
            endif()
        endif()
    endif()
...
function(him_add_nlohmann_json)
    if (HELLOIMGUI_USE_EXTERNAL_JSON)
        ...
        if(nlohmann_json_FOUND)
            ...
            if(HELLOIMGUI_INSTALL)
                install(FILES ${nlohmann_json_dir}/nlohmann/json.hpp DESTINATION include/nlohmann/json.hpp COMPONENT Development)
                install(FILES ${nlohmann_json_dir}/nlohmann/json_fwd.hpp DESTINATION include/nlohmann/json_fwd.hpp COMPONENT Development)
            endif()
        endif()
    endif()
endfunction()
